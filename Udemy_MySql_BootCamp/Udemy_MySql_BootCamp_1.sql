...
INSERT INTO films (name, lenght_min)
...
INSERT INTO seats (row_id, number, room_id)
...
select id from seats order by id asc;
...
delete from rooms where id in (5,6);

select * from films;
select name,max(lenght_min)from films;
...
select films.name,s.start_time,c.first_name,c.last_name,count(b.id)from films 
join screenings s on films.id = s.film_id
join bookings b on s.id = b.screening_id
join customers c on b.customer_id = c.id
group by films.name,c.last_name,c.first_name,s.start_time
order by s.start_time;


select b.customer_id,count(reserved_seat.id) from bookings b
join reserved_seat on b.id = reserved_seat.booking_id
group by b.customer_id;

select films.name,films.lenght_min,count(screenings.id)from films 
join screenings  on films.id = screenings.film_id
group by films.name,films.lenght_min
having films.lenght_min > 120;


sub queries - non correlated query
outter query
...
select name,lenght_min from films
where lenght_min >
(select avg(lenght_min)from films);
...
where film_id = f.id )from films f;
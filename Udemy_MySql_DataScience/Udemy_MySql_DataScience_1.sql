...
/*select * from employees
where email = 'null'; -- in any condition null is not equal to any value or null iself*/ 
...
select * from employees
where department in ('sports','firstaid','toys','garden');

select * from employees
where salary between '80000' and '100000';

select first_name,email from employees
where department = 'tools' and salary > 110000 and gender = 'f';
...
select * from employees
where department = 'automotive' and salary between '40000' and '100000' and gender = 'm'
...
/*select trim('  hello there     ');--- here it cleans the data it shows result with no spaces at start and end of words kind of data cleaning*/

/*select first_name || last_name -- this pipe operator is used to concat in sql but in my sql it is refered as 'or' logical operator
from employees;*/
/* BOOLEAN () EXPRESSIONS*/
/*select first_name,(salary > 140000) from employees;----here () this is a boolean which shows the result as true'1' or false'0'*/

 select first_name,(salary > 140000) from employees
 order by salary desc;
...
select department,('clothing' in(department,first_name)) from employees;--we can have multiple columns*/

select ('clothing'in('clothing','furniture','phones'));
/* STRING FUNCTIONS */
select substring('this is test data' from 1 for 4);
select substring('this is test data' from 1);
select substring('this is test data',1,4);

select department,replace(department,'Clothing','Attire'),concat(department,' ','department')
from departments;

select position('@' in email) from employees;
select substring(email, position('@' in email)) from employees;
/*select substring(email, position('@' in email) + 1) from employees; -- here it removes the @ as it counts from after @*/

/* select coalesce(email,'none') from employees -- replaces null values with 'none'*/
/* GROUPING FUNCTIONS */

select department,count(*) from employees
group by department;

select department,count(*),round(avg(salary)),min(salary),max(salary) from employees
group by department;
...
select gender,region_id,round(avg(salary)),min(salary),max(salary) from employees
group by gender,region_id
order by gender,region_id;

/* SUB QUERIES */
/*select * from employees
where department not in (select department from departments); -- here we are just giving a statement so no need for alias*/

/* select * from (select * from employees where salary > 150000) a -- every deliverable must have its own alias when that query is in from clause */

select a.first_name, a.salary from (select * from employees where salary > 150000) a;

select a.employee_name, a.yearly_salary
 from (select first_name as employee_name, salary as yearly_salary from employees where salary > 150000) a;
...
select first_name,last_name,salary,(select first_name from employees limit 1) from employees ;
 ...
 select * from regions; 
 select * from employees
 where salary > 130000 and region_id in (select region_id from regions where country in ('asia','canada')) ;
 
 select first_name,department,(select max(salary) from employees ) - salary as remaining from employees
...
where department in (select department from departments where division = 'kids')
 ...
 group by salary                      ---- here aggregate can be used anywhere in the query but not required to be in the select statement and the sequence in order by  is important
 ...
 */
 ...
 group by name;
       /*or*/
 select * from dupes
 where id in( select min(id) from dupes/* here in clause need only one column in subquery*/
 group by name);
 
 delete from dupes 
 where id not in (SELECT temp.id FROM ( select min(id)  id from dupes /* here delete function will not run on same table if used in subquery so we have to give an alias to the table in the subquery so that the outter query will treat the inner query operation result as a seperate table*/ 
 group by name) temp) ;
...
where salary not in ( (select max(salary)AS MAX from employees)
union 
  (select min(salary) AS MIN from employees));
         /* or */
  select avg(salary) from employees
  where salary not in ((select max(salary)AS MAX from employees),(select min(salary) AS MIN from employees));
                      /*  or  */
    select avg(salary) from employees
  where salary != (select max(salary)AS MAX from employees) and salary != (select min(salary) AS MIN from employees);
  
  select first_name,salary,
  case 
     when salary < 100000 then 'under paid'
     when salary > 100000 and salary < 160000 then 'paid well'/*-- if integer is used after 'then',we have to put integer after 'else'*/
...
case  when region_id   = 6 then (select country from regions where region_id = 6)  else 'null' end as region_6,
 case  when region_id   = 7 then (select country from regions where region_id = 7) else 'null' end as region_7
   ...
   select count(a.region_1)+count(a.region_2)+count(a.region_3) as united_states,count(a.region_4)+count(region_5)as asia,count(a.region_6)+count(region_7)as canada
   from( select first_name, case when region_id = 1 then (select country from regions where region_id = 1)  end as region_1,
...
case  when region_id   = 6 then (select country from regions where region_id = 6)    end as region_6,
 case  when region_id   = 7 then (select country from regions where region_id = 7)  end as region_7
   ...
   /* in the above query inner query is dependant on outter query */
   
   select first_name,department,salary,(select round(avg(salary)) from employees e2 where e1.department = e2.department)
   from employees e1;
...
(select max(salary) from employees e2 where e1.department  = e2.department) as maxdepartment,
(select min(salary) from employees e2 where e1.department  = e2.department) as mindepartment
from employees e1)a
where salary = maxdepartment
